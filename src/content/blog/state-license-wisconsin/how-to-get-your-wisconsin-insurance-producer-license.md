---
title: 'How to Get Your Wisconsin Insurance Producer License'
description: >-
  Wisconsin Insurance Producer License: Full Guide. Practical Wisconsin
  insurance guide for new and experienced agents. Get the rules, timelines,
  and...
keyword: wisconsin insurance producer license
targetKeywords: wisconsin insurance producer license
cluster: state-license-wisconsin
clusterName: 'State License – Wisconsin'
type: supporting
date: '2026-04-30'
author: Justin vom Eigen
image: /blog/images/get-ahead-with-the-best-insurance-prelicense-courses-today.jpg
imageAlt: >-
  Wisconsin insurance professional reviewing materials related to how to get
  your wisconsin insurance producer license.
---

Wisconsin's insurance licensing process has several features that make it genuinely distinctive among all comparison states. The Office of the Commissioner of Insurance (OCI) requires 20 hours of prelicensing education per line of authority, structured as an 8-hour common section (principles, WI laws, ethics — shareable across lines within 12 months) and a 12-hour line-specific section. PSI Services LLC administers the exam — the only comparison state using PSI alongside Minnesota and New Jersey. Fingerprinting is required through Fieldprint Wisconsin ($36). The application fee is $75 per line of authority, processing takes 1-3 business days, and there is no mandatory retake waiting period — allowing immediate rescheduling after a failed exam. Wisconsin's economy anchors on Northwestern Mutual (Fortune 500 life insurer; Milwaukee), Epic Systems (world's largest EHR company; Madison), American Family Insurance (Madison), and a manufacturing-and-dairy heritage that creates commercial insurance advisory need throughout the state. Here's the complete guide.

## Who Regulates Wisconsin Insurance

Office of the Commissioner of Insurance (OCI):

Mailing: PO Box 7872, Madison, WI 53707-7872

Physical: 101 E. Wilson St., Madison, WI 53703

Phone: 608-266-8699 (licensing); 800-236-8517 (consumer complaints)

Email: ociagentlicensing@wisconsin.gov

Website: oci.wi.gov

Insurance Code: Wisconsin Statutes, Chapters 600-655; Chapter 628 (intermediaries)

What OCI calls agents: Wisconsin uses the term Intermediary (Producer) Individual — not "agent" or "producer" as most states do. The license will display as "Intermediary (Producer) Individual" on the NIPR application and OCI records.

## Step 1: Complete Prelicensing Education (Required)

Wisconsin requires 20 hours of PLE per line of authority under Wis. Admin. Code Ch. Ins 26.

PLE structure:

8 hours: Principles of insurance + general Wisconsin insurance laws + ethics — common to all lines; if completed within a 12-month period, need not be repeated for each additional line

12 hours: Line-specific policies, terms, concepts, and line-specific insurance law

PLE characteristics:

Certification Exam: 70% minimum; monitored by disinterested third party (or currently licensed agent with no family/financial relationship); proctor must be physically present

PLE Certificate valid: 1 year from passing date

Bring Certificate of Completion to PSI exam (paper or electronic)

If exam passed BEFORE PLE completed: must retest with PSI

No combined exams in Wisconsin — Property and Casualty are separate exams, each requiring 20 hours of PLE; Life and Health are separate exams

PLE for multiple lines:

Life + Accident & Health: 8 hours (common) + 12 hours Life + 12 hours A&H = 32 hours total (8 hours shared)

Property + Casualty: 8 hours (common) + 12 hours Property + 12 hours Casualty = 32 hours total (8 hours shared)

Designation exemptions from PLE (OCI Form 11-026):

2-year Wisconsin technical college degree in insurance

4-year college degree in business with insurance emphasis

Veterans with insurance-related military instruction

Certain professional designations (see OCI exemption form OCI 11-026)

Must submit exemption request and receive approval 90 days prior to taking state exam

Submit Prelicensing Exemption Form OCI 11-026 to OCI

OCI study resource: "Intermediary's Guide to Wisconsin Insurance Law" — developed by OCI as a study guide for the WI laws section; available online at oci.wi.gov (no hard copies available)

JustInsurance's OCI-approved Wisconsin prelicensing courses are fully online and self-paced.

## Step 2: Complete Fieldprint Fingerprinting

Wisconsin requires digital fingerprinting through Fieldprint Wisconsin before or concurrently with the exam process.

Vendor: Fieldprint Wisconsin

Website: fieldprintwisconsin.com

Fieldprint code: FPWIOCIInsurance (not case sensitive)

Fee: $36.00 for digital fingerprinting

Two forms of current signature identification required at appointment (primary must be government-issued with photo)

Fingerprints valid for 180 days

Fingerprint cards NOT accepted; must use Fieldprint

Contact OCI if fingerprinted within 180 days for a previous WI insurance exam — may not need new prints

Schedule Fieldprint promptly — fingerprint processing contributes to background check completion before OCI issues the license.

## Step 3: Schedule and Pass the PSI Exam

Exam administrator: PSI Services LLC

Website: test-takers.psiexams.com/wiins

Phone: (888) 818-5805

Remote testing: PSI Bridge (check compatibility at syscheck.bridge.psiexams.com; requires Google Chrome)

Spanish exams available

Schedule at least 2 days in advance

Exam fee: $75 per exam (each line is a separate exam) Passing score: 70% Score validity: Apply within 180 days of passing exam

Results: Score displayed on screen at end; email report sent. Diagnostic report emailed if failed.

Retakes: Unlimited; no mandatory waiting period — can immediately reschedule.

Cancel/reschedule: At least 48 hours in advance or forfeit fee.

Exam day: Arrive 30 minutes early; bring: (1) government-issued photo ID; (2) PLE Certificate of Completion.

First-time candidates: Must select "General and State Specific" exam. Previously licensed agents may take "State Specific Series" only.

Wisconsin Exam Specifications:

## Step 4: Submit License Application

After passing exam and completing fingerprinting:

Apply through NIPR (nipr.com):

Application fee: $75 per line of authority

NIPR transaction fee: $5.60

Wait 48-72 hours after passing exam before submitting electronic application

Apply within 180 days of passing exam

90 days to submit required documentation after application — or application expires

Processing: 1-3 business days (24-48 hours for clean applications).

CE reporting fee: $1.00 per credit hour when reporting CE completions.

Temporary license: 12-month temporary license available at $75 per line — only for personal representatives of deceased/disabled agents or agents entering military active duty.

Veterans: Exam fee reimbursement available; initial license fee waiver available — see OCI Veterans Fee Waiver page.

## Step 5: Maintain Your License

Renewal: 2 years; last day of birth month. Renewal fee: $35 (resident); $75 (nonresident). CE: 24 hours (3 Ethics); no carryover of excess hours. Renewal notifications: Email only (eff. February 1, 2021) to business email on file within 60 days of expiration.

## Wisconsin vs. Comparison States

## 5 Frequently Asked Questions

- What makes Wisconsin's 8-hour shared PLE section distinctive? Wisconsin's 8-hour common section — covering principles of insurance, general Wisconsin insurance laws, and ethics — is required for every line but needs to be completed only once within a 12-month period. If you complete Life PLE in January and want to add Health PLE in September of the same year, you only need the 12-hour Health-specific section (not the full 20 hours again). This is more efficient than states like Indiana and Missouri (where each line's PLE is fully independent) and allows multi-line candidates to build credentials progressively.
- Why does Wisconsin have no mandatory retake waiting period? Wisconsin's PSI exam allows candidates to immediately reschedule after a failed attempt — no mandatory 24-hour or 48-hour wait. This is the most candidate-friendly retake policy of all comparison states (Indiana requires 48 hours; Missouri requires 24 hours; Maryland requires 4 days). The immediate reschedule option means candidates who fail can review their diagnostic report, study focused weak areas, and test again on the next available appointment without administrative delays.
- What is the OCI's "Intermediary's Guide to Wisconsin Insurance Law"? OCI developed this study guide specifically for the Wisconsin laws section of the PSI exam — covering insurance concepts, state laws, and ethics. Hard copies are no longer printed; candidates access it online at oci.wi.gov. It is the only OCI-published study material for the Wisconsin laws portion — OCI has not prepared study materials for the product knowledge (national) questions. Every WI exam prep candidate should review this guide alongside their PLE course.
- What is the Wisconsin 180-day score validity and how does it create a timeline? After passing the PSI exam, candidates must apply for the license within 180 days. This includes time for Fieldprint fingerprinting processing and OCI application review (1-3 business days). The 180-day window is substantially more generous than Indiana's 6-month PLE window but less forgiving than Missouri's 1-year window. Since OCI processes applications in 1-3 business days, there's no time pressure from processing — the fingerprint results must simply be received before OCI issues the license.
- Does Wisconsin allow combined Life & Health or P&C exams like some other states? No — Wisconsin explicitly does NOT offer combined exams. Property and Casualty are separate exams (each $75; each 100 questions; each 2 hours), requiring separate study and separate PLE tracks (8 hours shared + 12 hours each). Life and Accident & Health are similarly separate. Candidates pursuing all four major lines must take four separate PSI exams at $75 each = $300 in exam fees (vs. Missouri's combined L&H + P&C approach at $70 total).

## Start Your Wisconsin License Process

Wisconsin's OCI framework, PSI exam network, and no-wait retake policy create a clear and achievable licensing timeline. JustInsurance's OCI-approved Wisconsin prelicensing courses cover all four major lines with the 8-hour shared section built in.

Enroll today and start your Wisconsin insurance career.
