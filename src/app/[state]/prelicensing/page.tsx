...
export async function generateMetadata({
  ...
  if (stateData.slug === "hawaii") {
    const description =
      "Optional Hawaii Life, Accident & Health, and combined insurance exam preparation. Study online with 30-day access and optional weekly instructor support.";
    return {
      ...baseMeta,
      description,
  ...
  if (stateData.slug === "idaho") {
    const description =
      "Optional Idaho Life, Disability/Health, and combined insurance exam preparation. Study online with 30-day access and optional weekly instructor support.";
    return {
      ...baseMeta,
      description,
      openGraph: { ...baseMeta.openGraph, description },
  ...
  if (isPrelicensingPartiallyLive(stateData)) {
    const title = stateData.slug === "new-york"
      ? "New York Insurance Prelicensing Courses"
      : `${stateData.name} Life Insurance Prelicensing Course`;
    const description = `The approved ${stateData.name} Life prelicensing course is open for enrollment. Health and combined prelicensing are not currently available from JustInsurance.`;
    return {
      ...baseMeta,
      title,
      description,
...
export default async function PrelicensingHubPage({
  ...
  // Delaware also has no prelicensing prerequisite and its live products are
  // optional exam preparation with optional weekly instructor support.
  const isDelaware = stateData.slug === "delaware";
  const isOptionalExamPrep = isAlabama || isAlaska || isArizona || isDelaware;
  // Hawaii likewise has no prelicensing prerequisite. Keep the established
  // SEO title/H1 for this rollout, but use optional-exam-prep wording anywhere
  // the page describes course completion or the buyer's required steps.
  const isHawaii = stateData.slug === "hawaii";
  // Idaho does not require prelicensing education and has separate Life and
  // Disability/Health licensing exams. Preserve the established route/title,
  // while correcting buyer-facing course and completion language.
  const isIdaho = stateData.slug === "idaho";
  // These rollout pages keep their established SEO titles and H1s,
  // but the buyer-facing steps must not imply that optional exam preparation
  // produces a state-required licensing certificate.
  const isNewOptionalRollout = ["iowa", "kansas", "louisiana", "maine", "maryland", "massachusetts"].includes(stateData.slug);
  const isOptionalProductFlow = isOptionalExamPrep || isHawaii || isIdaho || isNewOptionalRollout;
  ...
  const articleDescription = prelicensingPartiallyLive
    ? `The approved ${stateData.name} Life prelicensing course is open for enrollment. Health and combined prelicensing are not currently available from JustInsurance.`
    : isCEOnlyState
  ...
  return (
    <>
      ...
      <StateHero
        ...
        subtitle={
          ...
          prelicensingPartiallyLive
            ? `The approved ${stateData.name} Life prelicensing course is open for enrollment. Health and combined prelicensing courses are not currently available from JustInsurance.`
            : ilWebinar
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-navy mb-4">
                {isOptionalProductFlow ? "What is Insurance Exam Prep?" : "What is Insurance Prelicensing?"}
              </h2>
              <p className="text-gray-600 leading-relaxed mb-4">
                {(() => {
                  ...
                  if (isHawaii) {
                    return `Hawaii does not require prelicensing education before its insurance licensing exams. JustInsurance's Hawaii courses are optional exam preparation organized around the separate Life and Accident & Health or Sickness exam topics, with structured lessons, practice, and instructor support.`;
                  }
                  if (isIdaho) {
                    return `Idaho does not require prelicensing education before its insurance licensing exams. JustInsurance's Idaho courses are optional exam preparation organized around the separate Life and Disability/Health exam topics, with structured lessons, practice, and instructor support.`;
                  }
              ...
              <p className="text-gray-600 leading-relaxed mb-4">
                {ilWebinar ? (
                ...
                ) : stateData.slug === "florida" || stateData.slug === "connecticut" || isArkansas ? (
                  ...
                  <>JustInsurance&apos;s Colorado prelicensing courses are fully online and self-paced. You can study on any device, at any time. Optional weekly live instructor sessions are available for extra support, but attendance is not required.</>
                ) : prelicensingPartiallyLive ? (
                  <>The currently available JustInsurance {stateData.name} Life prelicensing course is delivered online and includes 30 days of access. Health and combined Life &amp; Health prelicensing are not currently available from JustInsurance.</>
                ) : (
                  <>JustInsurance&apos;s {stateData.name} prelicensing courses are fully online and self-paced. You can study on any device, at any time. There are no required live sessions to attend — just work through the lessons at your own pace and move on when you&apos;re ready.</>
                )}
              </p>
              <p className="text-gray-600 leading-relaxed">
                {ilWebinar ? (
                  ...
                  <>Health and combined Life &amp; Health applicants receive a certificate after completing the required hours and course assessments. Life-only applicants may use the Life course as optional exam preparation, but Mississippi does not require its completion before the Life licensing exam. You schedule the state licensing exam separately with {stateData.examInfo.examProvider}.</>
                ) : (
              ...
              {[
                {
                  step: "1",
                  title: "Enroll Online",
                  desc: prelicensingPartiallyLive
                    ? `Enroll in the currently available ${stateData.name} Life course. Health and combined prelicensing are not currently available from JustInsurance.`
                    : "Choose your line of authority and enroll in minutes. Instant access to all course materials.",
                },
                ...
                ilWebinar
                  ...
                  : { step: "2", title: "Study at Your Pace", desc: isCEOnlyState ? `Work through the exam-prep course at your own pace for your line of authority — video lessons, readings, and quizzes on any device.` : isMississippi ? "Complete the required hours for Health or combined Life & Health, or use the Life-only course as optional exam preparation." : `Complete your required prelicensing hours for your line of authority — video lessons, readings, and quizzes on any device.` },
                ilWebinar
                  ...
                  : { step: "3", title: "Earn Your Certificate", desc: isMississippi ? "Health and combined applicants receive a certificate after completing the required course; Life-only applicants do not need a course certificate." : "Pass the course assessments to receive your official certificate of completion from JustInsurance." },
                ...(isArkansas
                  ? [
            ...
            {[
              {
                ...
                desc: prelicensingPartiallyLive
                  ? `Enroll in the currently available ${stateData.name} Life course online. Health and combined prelicensing are not currently available from JustInsurance.`
                  : "Choose your line of authority (Life, Health, or Life & Health) and enroll online.",
              },
              {
                step: "2",
                icon: "📚",
                title: isOptionalProductFlow ? "Complete Your Exam Prep" : "Complete Your Education",
                ...
                desc: ilWebinar
                  ...
                  : isMississippi
                  ? "Complete the required Health or combined course, or use the Life-only course as optional online exam preparation."
      ...
      {/* California's detailed provider/course disclosure remains visible and
          crawlable, but sits lower in the page so it does not dominate the
          first mobile viewport. Other states keep their existing placement. */}
      {stateData.slug === "california" && (
        <PrelicenseApprovalNotice stateSlug={stateData.slug} stateName={stateData.name} />
      )}
      ...
      <FAQAccordion
        faqs={faqs}
        heading={`${stateData.name} ${isOptionalProductFlow ? "Insurance Exam Prep" : "Prelicensing"} FAQs`}
      />
