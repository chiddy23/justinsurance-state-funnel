...
export interface StateDataForFAQ {
  ...
  // Gates PRELICENSING "state-approved" claims (paired with prelicensingRequired).
  // NOTE: approval alone no longer authorizes a CE PURCHASE or a live CE claim —
  // an approved provider can still have CE courses that are not open yet (WA, NY).
  // Use ceAvailable for anything that sells CE or asserts live CE reporting.
  providerApproved: boolean;
  // Raw provider approval number (e.g. WA "300632", NY "80025") or "PENDING".
  // Surfaced only to name the number in an "Approved — courses coming soon" notice.
  providerApprovalNumber: string;
  // CE courses are approved AND live/purchasable (isCeAvailable). This is the ONLY
  // condition under which a CE enroll/buy claim or a live "state-approved CE" /
  // "we report to the DOI" statement may render.
  ceAvailable: boolean;
  // Complete CE package availability. May be false while approved individual
  // CE courses remain public (Illinois during the 2026-08 rollout audit).
  cePackagesAvailable: boolean;
  // Approved CE provider but courses not open for enrollment yet
  // (isCeApprovedComingSoon) → render "Approved [State] CE provider (#number) —
  // courses coming soon", NOT a package/price/purchase claim and NOT "approval
  // pending". When neither ceAvailable nor ceApprovedComingSoon is true the state
  // is genuinely pending (none right now) and the "approval pending" copy applies.
  ceApprovedComingSoon: boolean;
  // Safe to assert a LIVE "state-approved [State] prelicensing course" claim:
  // provider approved AND the state regulates prelicensing AND the prelicensing
  // course is live (stateClaims().canClaimPrelicensingApproval). FALSE for a
  // prelicensing-approved-but-not-live state (New York), so hub copy falls back
  // to the neutral "built to the exam content outline" wording instead of
  // asserting we already offer approved prelicensing courses there.
  prelicensingApprovedLive: boolean;
  // Prelicensing-required state whose prelicensing course is NOT live yet
  // (isPrelicensingHeld) — currently New York only. TRUE suppresses every hub
  // prelicensing OFFER/price/package sentence and swaps in "opening for
  // enrollment soon". Deliberately NOT prelicensingApprovedLive: that field is
  // false for exam-only LIVE states (Texas), which DO sell live exam-prep and
  // must keep their offer copy byte-identical. Only a HELD state suppresses.
  prelicensingHeld: boolean;
  // A verified subset of the state's lines is live (New York: Life only).
  // This prevents FAQ copy and FAQ schema from advertising unavailable lines.
  prelicensingPartiallyLive: boolean;
...
export function buildFaqData(stateData: StateData): StateDataForFAQ {
  ...
  return {
    ...
    ceAvailable: isCeAvailable(stateData),
    cePackagesAvailable:
      isCeAvailable(stateData) && stateData.cePackagesLive !== false,
    ceApprovedComingSoon: isCeApprovedComingSoon(stateData),
    prelicensingApprovedLive: stateClaims(stateData).canClaimPrelicensingApproval,
    prelicensingHeld: isPrelicensingHeld(stateData),
    prelicensingPartiallyLive: isPrelicensingPartiallyLive(stateData),
    prelicensingLiveLines: stateData.prelicensingLiveLines,
...
export function getStateHubFAQs(data: StateDataForFAQ): FAQ[] {
  const isIllinois = data.name === "Illinois";
  const partialPrelicensing = data.prelicensingPartiallyLive;
  ...
  const approvedAdj = data.prelicensingApprovedLive ? "state-approved " : "";
  ...
  return [
    {
      ...
      answer: `Getting your ${data.name} insurance license involves ${stepCountWords[licenseStepClauses.length]} steps. ${licenseStepsSentence} Most candidates complete the entire process in ${data.totalLicensingTime}. ${data.prelicensingHeld ? `JustInsurance's ${data.name} prelicensing courses are opening for enrollment soon.` : partialPrelicensing ? `JustInsurance currently offers its approved ${data.name} Life prelicensing course online. Health and combined Life & Health prelicensing are not currently available from JustInsurance.` : `JustInsurance offers fully online, self-paced ${prelicensingRequired ? "prelicensing" : "exam-prep"} courses ${data.prelicensingApprovedLive ? `approved for ${data.name}` : `built to the ${data.name} exam content outline`} that get you to exam day confident and prepared.`}`,
    },
    {
      question: `How much does it cost to get ${aOrAn(data.name)} ${data.name} insurance license?`,
      answer: `The total cost to get your ${data.name} insurance license typically runs ${data.totalCostRange}. Here is how it breaks down: the ${data.examProvider} exam fee is ${$(data.examFee)}, the ${data.doiName} application fee is ${$(data.applicationFee)}${data.backgroundRequirement.toLowerCase().includes("required") ? ", and a background check adds a small additional cost" : ""}. ${data.prelicensingHeld ? `JustInsurance's ${data.name} prelicensing courses are opening for enrollment soon.` : partialPrelicensing ? `The currently available JustInsurance ${data.name} Life prelicensing course is ${$(data.lifePrice)}. Health and combined Life & Health prelicensing are not currently available from JustInsurance.` : isIllinois ? `JustInsurance's Illinois Life and Accident & Health prelicensing courses are sold separately at ${$(data.lifePrice)} each; students pursuing both lines currently pay ${$(data.combinedPrice)} for the two courses. Each line also has its own licensing exam.` : `If you need a prelicensing course, JustInsurance's ${data.name} ${noCombinedCourse ? "Life & Health package (the Life course and the Accident & Health course together) is" : "combined Life & Health package is"} ${$(data.combinedPrice)} — that's ${$(data.combinedSavings)} less than purchasing Life and Health courses separately. Everything you need to get licensed, with no hidden fees.`}`,
    },
    {
...
export function getPrelicensingHubFAQs(data: StateDataForFAQ): FAQ[] {
  ...
  } else if (partialPrelicensing) {
    hoursAnswer = `${data.name} requires ${formatHours(data.lifeHours)} of approved prelicensing education for Life, ${formatHours(data.healthHours)} for Accident & Health, or ${formatHours(data.combinedHours)} for the combined Life, Accident & Health authority. JustInsurance currently offers its approved ${data.name} Life prelicensing course. Health and combined Life & Health prelicensing are not currently available from JustInsurance.`;
  } else {
    const parts: string[] = [];
  ...
  const coverAnswer = partialPrelicensing
    ? `The currently available JustInsurance ${data.name} Life prelicensing course covers life-insurance topics in the published licensing outline, including policy types, provisions, annuities, underwriting, beneficiaries, state insurance law, ethics, and consumer protections. Health and combined Life & Health prelicensing are not currently available from JustInsurance.`
    : isOptionalExamPrep
  ...
  return [
    ...
    },
    {
      ...
      answer: partialPrelicensing
        ? `Yes. The currently available JustInsurance ${data.name} Life prelicensing course is delivered online and includes ${data.courseAccessDays} days of access. Complete the required course work and receive your completion certificate before following the current ${data.examProvider} exam-registration process. Health and combined Life & Health prelicensing are not currently available from JustInsurance.`
        : isCalifornia
        ...
        : isIllinois
        ? `Yes. JustInsurance's Illinois prelicensing courses are delivered online, but they are not entirely self-paced. Illinois requires 7.5 of the 20 hours for each line of authority to be completed in a live classroom or webinar with verified attendance; the remaining 12.5 hours per line are self-paced. The mandatory live webinar must be attended before your certificate of completion can be issued. Your enrollment includes both portions and provides ${data.courseAccessDays} days of access.`
        : isMinnesota
    ...
    },
    {
      ...
      answer: partialPrelicensing
        ? `The currently available JustInsurance ${data.name} Life prelicensing course costs ${$(data.lifePrice)} and includes the required Life course content, chapter reviews, additional practice questions based on published licensing topics, ${data.courseAccessDays} days of access, and a completion certificate after you satisfy the course requirements. Health and combined Life & Health prelicensing are not currently available from JustInsurance.`
        : isOptionalExamPrep
        ...
        : isIllinois
        ? `JustInsurance offers a ${$(data.combinedPrice)} Illinois Life & Health package that enrolls you in both separately approved 20-hour courses in one purchase. Each course includes 7.5 mandatory live webinar hours plus 12.5 self-paced hours and produces its own completion certificate. Individual Life-only and Accident & Health-only courses remain available at ${$(data.lifePrice)} each. Your ${data.courseAccessDays}-day access window starts when you enroll.`
  ...
  return [
    {
      ...
      answer: `${data.name} requires ${data.ceTotalHours} hours of continuing education (CE) every ${data.ceRenewalPeriod} to maintain an active insurance producer license.${data.ceEthicsHours > 0 ? ` Of those ${data.ceTotalHours} hours, ${data.ceEthicsHours} must specifically cover ethics and professional conduct — this is a mandatory component, not optional.` : ``}${data.ceMandatedTopicHours ? ` ${data.ceMandatedTopicHours}` : ``}${data.ceLawChangeHours && !data.ceMandatedTopicHours ? ` ${data.name} also requires at least ${data.ceLawChangeHours} of those ${data.ceTotalHours} hours to cover changes in ${data.name} insurance statutes and administrative rules — a distinct mandatory topic, separate from ethics.` : ``}${firstTerm ? ` Your first renewal is different: if you are newly licensed, ${data.name} requires ${firstTerm} CE hours before your FIRST renewal (${data.ceEthicsHours > 0 ? `still including the ${data.ceEthicsHours} ethics hours; ` : ``}not ${data.ceTotalHours}), then ${data.ceTotalHours} hours every ${data.ceRenewalPeriod} for each renewal after that.` : ``} All CE must be completed through a ${data.doiName}-approved provider before your license renewal deadline. ${data.cePackagesAvailable ? `JustInsurance is an approved provider and offers a complete ${data.ceTotalHours}-hour CE package for ${data.cePackagePrice}${data.ceEthicsHours > 0 ? `, including the required ethics hours` : ``}.${firstTerm ? ` That package covers the ${data.ceTotalHours} hours required for a standard renewal cycle, so if you are heading into your first renewal you need ${firstTermExtra} additional CE hours to reach ${firstTerm}. Confirm your own requirement with the ${data.doiName}.` : ``}` : data.ceAvailable ? `JustInsurance is an approved ${data.name} CE provider and currently offers individual CE courses; a complete ${data.ceTotalHours}-hour package is not currently available.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}) — our ${data.name} CE courses are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending; our ${data.name} CE courses are not yet available.`}${data.ceLiveInstructorHours ? ` Separately, at least ${data.ceLiveInstructorHours} of the ${data.ceTotalHours} hours must be earned through a formal classroom or another format that lets you interact with a live instructor (13.4.7 NMAC); a fully self-paced package does not satisfy that live-instructor component on its own.` : ``}`,
    },
    {
      question: `How much does ${data.name} insurance CE cost?`,
      answer: data.cePackagesAvailable
        ? `JustInsurance offers a complete ${data.name} CE package — covering ${firstTerm ? `the ${data.ceTotalHours} hours required for a standard renewal cycle` : data.ceLiveInstructorHours ? `the self-paced portion of your ${data.ceTotalHours}-hour requirement` : `all ${data.ceTotalHours} required hours`}${data.ceEthicsHours > 0 ? ` including the ${data.ceEthicsHours}-hour ethics requirement` : ``} — for ${data.cePackagePrice}. This is a flat-fee bundle with no per-course upsells or surprise charges. Individual CE courses are also available for agents who need only specific topics to round out their renewal hours${firstTerm ? `, including the ${firstTermExtra} extra hours a newly licensed producer needs to reach the ${firstTerm}-hour first-renewal requirement` : ``}. Every purchase includes immediate online access, self-paced completion, and automatic CE credit reporting to the ${data.doiName} on your behalf.`
        : data.ceAvailable
        ? `JustInsurance currently offers approved individual ${data.name} CE courses for agents who need specific topics or remaining hours. A complete ${data.ceTotalHours}-hour package is not currently available, so there is no package price to quote. Review your producer transcript and confirm the courses you select satisfy your remaining hours and the ${data.ceEthicsHours}-hour live ethics requirement.`
        : data.ceApprovedComingSoon
        ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}), but our ${data.name} CE courses are coming soon and not yet open for enrollment — there is no ${data.name} CE package or price to quote yet. ${data.name} requires ${data.ceTotalHours} hours of CE every ${data.ceRenewalPeriod}${data.ceEthicsHours > 0 ? `, including ${data.ceEthicsHours} hours of ethics` : ``}; check back soon, or verify your requirement with the ${data.doiName}.`
        : `JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} CE courses are not yet available and we cannot quote a package or price. ${data.name} requires ${data.ceTotalHours} hours of CE every ${data.ceRenewalPeriod}${data.ceEthicsHours > 0 ? `, including ${data.ceEthicsHours} hours of ethics` : ``}; verify your requirement with the ${data.doiName}.`,
    },
    {
      question: `Can I complete my ${data.name} insurance CE online?`,
      answer: data.ceAvailable
        ? `Yes. ${data.name} approves online CE courses, and JustInsurance provides a full catalog of self-paced courses that meet all ${data.doiName} requirements. You can study on any device, pause and resume whenever you need to,${data.name === "Illinois" ? ` and work through the self-paced portion of your ${data.ceTotalHours}-hour requirement in a single sitting or across multiple sessions; note that Illinois requires the ${data.ceEthicsHours} mandatory ethics hours to be completed in a live classroom or webinar (synchronous) session with verified attendance — self-paced online study does not satisfy the ethics requirement, so those hours must be a scheduled live session (215 ILCS 5/500-35(b)(1)).` : ` and complete your ${firstTerm ? `${data.ceTotalHours}-hour renewal-cycle requirement (${firstTerm} hours before your first renewal)` : `${data.ceTotalHours}-hour requirement`} in a single sitting or spread across multiple sessions.`}${data.slug === "utah" ? ` One Utah limit to plan around: only 12 of the 24 hours may be self-study — at least 12 must be classroom or classroom-equivalent courses, which in Utah includes live webinars and timed courses (${data.doiName} CE requirements; Utah Admin. Code R590-142 defines the instruction methods).` : ``} Once you finish each course, JustInsurance reports your completion electronically to the ${data.doiName} — you do not need to mail certificates or manually update your license record.`
        : data.ceApprovedComingSoon
        ? `${data.name} approves online CE courses, and JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}) — but our online ${data.name} CE catalog is coming soon and not yet open for enrollment. Once our courses go live you will be able to complete your ${data.ceTotalHours}-hour requirement online and we will report your completions electronically to the ${data.doiName}. Until then, verify your renewal requirement with the ${data.doiName}.`
        : `${data.name} approves online CE courses. JustInsurance's ${data.name} CE provider approval is currently pending, so our online ${data.name} CE catalog is not yet available. Once ${data.name} finalizes our approval, you will be able to complete your ${data.ceTotalHours}-hour requirement online and we will report your completions electronically to the ${data.doiName}.`,
    },
    {
      question: `What happens if I don't complete my ${data.name} CE on time?`,
      answer: data.ceLapseConsequence
        ? `${data.ceLapseConsequence} Specifically in ${data.name}: the grace period is ${data.ceGracePeriod}, the late fee is ${data.ceLateFee}, and reinstatement runs ${data.ceReinstatementFee} with a window of ${data.ceReinstatementWindow}. A lapsed license also interrupts your commission flow and can trigger carrier appointment reviews. ${data.cePackagesAvailable ? `JustInsurance makes it easy to finish your hours well before your deadline — the entire ${data.cePackagePrice} package can be completed in a single day if needed.` : data.ceAvailable ? `JustInsurance offers approved individual CE courses; verify your remaining requirements before choosing them.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}); our ${data.name} CE courses are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending; our ${data.name} CE courses are not yet available.`}`
        : `If you fail to complete your ${data.ceTotalHours} CE hours and renew your license before your deadline, your ${data.name} insurance license will lapse. A lapsed license means you cannot legally sell, solicit, or negotiate insurance until it is reinstated. Reinstatement typically requires paying a late fee, submitting a reinstatement application to the ${data.doiName}, and in some cases completing additional CE hours beyond the standard ${data.ceTotalHours}. A lapsed license can also interrupt your commission flow and harm your carrier appointments. ${data.cePackagesAvailable ? `JustInsurance makes it easy to finish your hours well before your deadline — the entire ${data.cePackagePrice} package can be completed in a single day if needed.` : data.ceAvailable ? `JustInsurance offers approved individual CE courses; verify your remaining requirements before choosing them.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}); our ${data.name} CE courses are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending; our ${data.name} CE courses are not yet available.`}`,
    },
    ...(data.ceReinstatementFee
      ? [
          {
            question: `How do I reinstate a lapsed ${data.name} insurance license?`,
            answer: `If your ${data.name} insurance license has lapsed, you typically have ${data.ceReinstatementWindow} to reinstate it before you must re-apply as a new licensee. Reinstatement in ${data.name} runs ${data.ceReinstatementFee}, and you must complete any outstanding CE hours from the lapsed period before the ${data.doiName} will process your reinstatement. The sooner you act, the simpler the process — waiting too long can mean starting over${ceHubPrelicensingRequired ? ` with prelicensing education and the state exam` : ` and retaking the state exam`} from scratch.${data.ceAvailable ? ` JustInsurance courses report electronically to the ${data.doiName}, so once your hours are complete your reinstatement moves fast.` : ``}`,
          },
        ]
      : []),
    ...(data.ceCarryForward
      ...
      : []),
    {
      ...
      answer: data.ceEthicsHours > 0
        ? `Yes. ${data.name} mandates that ${data.ceEthicsHours} of your ${firstTerm ? `required CE hours (${data.ceTotalHours} per renewal cycle, ${firstTerm} before your first renewal)` : `${data.ceTotalHours} required CE hours`} specifically cover ethics and professional conduct. This is not optional — you must satisfy the ethics component separately from general CE hours, and the ${data.doiName} tracks it independently.${data.ceMandatedTopicHours ? ` Ethics is not the only subject ${data.name} mandates: ${data.ceMandatedTopicHours}` : ``}${data.ceLawChangeHours && !data.ceMandatedTopicHours ? ` This is separate from ${data.name}'s requirement that at least ${data.ceLawChangeHours} of those ${data.ceTotalHours} hours cover changes in ${data.name} insurance statutes and administrative rules.` : ``} Failing to complete the ethics hours is treated the same as failing to complete your total CE requirement. ${data.cePackagesAvailable ? `JustInsurance's ${data.cePackagePrice} CE package includes a fully approved ${data.ceEthicsHours}-hour ethics course, so ${firstTerm ? `your ethics requirement is covered in the same purchase — add ${firstTermExtra} more hours if you are completing your ${firstTerm}-hour first renewal` : data.ceLiveInstructorHours ? `your ethics requirement is covered in the same purchase — note ${data.name} also requires at least ${data.ceLiveInstructorHours} of the ${data.ceTotalHours} hours through a formal classroom or live-instructor format (13.4.7 NMAC), which a self-paced package does not include` : `your entire renewal requirement is covered in one purchase`}.` : data.ceAvailable ? `JustInsurance offers approved individual CE courses, but no complete package is currently available. Confirm that the courses you select include ${data.name}'s required ${data.ceEthicsHours}-hour live classroom or webinar ethics course.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}); our ${data.name} CE courses, including the required ${data.ceEthicsHours}-hour ethics course, are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} CE courses — including the required ${data.ceEthicsHours}-hour ethics course — are not yet available.`}`
        : `No. ${data.name} does not set aside a dedicated ethics-hours requirement — none of your ${data.ceTotalHours} CE hours per ${data.ceRenewalPeriod} have to be ethics-specific, though ethics and professional-conduct courses still count toward your total. ${data.ceAvailable ? `JustInsurance's ${data.cePackagePrice} CE package covers your ${firstTerm ? `${data.ceTotalHours}-hour standard renewal requirement (a first renewal takes ${firstTerm} hours)` : `full ${data.ceTotalHours}-hour renewal requirement`}, with ethics and professional-responsibility material included as part of the curriculum.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}) — our ${data.name} CE courses are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} CE courses are not yet available.`}`,
    },
  ];
}
  ...
  return [
    {
      ...
      }` : data.ceAvailable ? `JustInsurance currently offers approved individual ${data.name} CE courses, but a complete ${loaName} package is not currently available. Review your producer transcript and confirm each selected course satisfies your remaining hours and required topics.` : data.ceApprovedComingSoon ? `The JustInsurance ${data.name} ${loaName} CE course will cover ${hours} hours toward that total once it launches. JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}), but our ${data.name} CE courses are coming soon and not yet open for enrollment — confirm your own requirement with the ${data.doiName}.` : `The JustInsurance ${data.name} ${loaName} CE course will cover ${hours} hours toward that total once it launches. JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} CE courses are not yet available — confirm your own requirement with the ${data.doiName}.`}`,
    },
    {
      question: `How long do I have to complete my ${data.name} CE?`,
      answer: `Your ${data.name} insurance license renews every ${data.ceRenewalPeriod}, and ${firstTerm ? `all ${firstTerm} CE hours must be completed before your FIRST renewal deadline, then ${data.ceTotalHours} hours before each renewal deadline after that` : `all ${data.ceTotalHours} CE hours must be completed before your renewal deadline`}. Your exact renewal deadline is set by the ${data.doiName} — log in to your producer portal to confirm it. JustInsurance recommends finishing your CE at least 30 days before your renewal date${data.ceAvailable ? ` to allow time for electronic reporting to process` : ``}. ${data.cePackagesAvailable ? `If you are cutting it close, the JustInsurance ${data.name} CE course can be completed in a single day.` : data.ceAvailable ? `Approved individual courses are available; completion time depends on the courses and hours you still need.` : data.ceApprovedComingSoon ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}) — our ${data.name} CE courses are coming soon and not yet open for enrollment.` : `JustInsurance's ${data.name} CE provider approval is currently pending; our ${data.name} CE courses are not yet available.`}`,
    },
    {
      ...
      answer: data.ceAvailable
        ...
        : data.ceApprovedComingSoon
        ? `Not yet. JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}), but our ${data.name} CE courses are coming soon and not yet open for enrollment. Once they go live, we will electronically report your completions directly to the ${data.doiName} — typically within one business day — so you will not need to mail certificates or update your license record manually.`
    ...
    },
    {
      ...
      answer: data.ceAvailable
        ? `Yes — and JustInsurance strongly recommends it. You can begin your ${data.name} CE courses at any point during your ${data.ceRenewalPeriod} renewal cycle. Starting early gives you the flexibility to spread your ${firstTerm ? `${data.ceTotalHours} hours (${firstTerm} if you are working toward your first renewal)` : `${data.ceTotalHours} hours`} across multiple sessions, choose topics relevant to your practice, and avoid any last-minute technical issues. Completed CE hours are reported to the ${data.doiName} as you finish each course, so credits accumulate throughout the cycle rather than all at once at the end.`
        : data.ceApprovedComingSoon
        ? `You can complete your ${data.name} CE at any point during your ${data.ceRenewalPeriod} renewal cycle, and starting early is smart. JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}), but our ${data.name} CE courses are coming soon and not yet open for enrollment — check back soon.`
        : `You can complete your ${data.name} CE at any point during your ${data.ceRenewalPeriod} renewal cycle, and starting early is smart. JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} CE courses are not yet available.`,
    },
    {
      question: `What topics are covered in the ${data.name} ${loaName} CE course?`,
      answer: data.cePackagesAvailable
        ? `The JustInsurance ${data.name} ${loaName} CE course covers the core topics required to keep your knowledge current and your license compliant. Expect content on ${loaName.toLowerCase()} product updates and regulatory changes specific to ${data.name}, consumer protection requirements, claims handling and suitability standards, ${data.name} insurance statutes governing producer conduct, and ethics and professional responsibility${data.ceEthicsHours > 0 ? ` — satisfying the mandatory ${data.ceEthicsHours}-hour ethics component` : ``}. All course content is reviewed and updated whenever the ${data.doiName} revises its approved topic list, so you are always studying current material.`
        : data.ceAvailable
        ? `Approved individual ${data.name} CE courses cover different topics and hour amounts. Review each course description and your producer transcript before enrolling to confirm the selected courses satisfy your remaining ${loaName} hours and the mandatory ${data.ceEthicsHours}-hour live ethics component.`
        : data.ceApprovedComingSoon
        ? `JustInsurance is an approved ${data.name} CE provider (#${data.providerApprovalNumber}), and our ${data.name} ${loaName} CE course is coming soon and not yet open for enrollment. When it launches it will cover ${loaName.toLowerCase()} product updates and regulatory changes specific to ${data.name}, consumer protection requirements, claims handling and suitability standards, ${data.name} insurance statutes governing producer conduct, and ethics and professional responsibility${data.ceEthicsHours > 0 ? ` — satisfying the mandatory ${data.ceEthicsHours}-hour ethics component` : ``}.`
        : `JustInsurance's ${data.name} CE provider approval is currently pending, so our ${data.name} ${loaName} CE course is not yet available. When it launches it will cover ${loaName.toLowerCase()} product updates and regulatory changes specific to ${data.name}, consumer protection requirements, claims handling and suitability standards, ${data.name} insurance statutes governing producer conduct, and ethics and professional responsibility${data.ceEthicsHours > 0 ? ` — satisfying the mandatory ${data.ceEthicsHours}-hour ethics component` : ``}.`,
    },
  ];
}
